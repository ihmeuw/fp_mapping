  ...
  if (adm0_type == "detect") {
    if (is.null(shapefile_version)) {
      adm0_type <- detect_adm_shapefile_date_type(
        lsae.utils::get_admin_shapefile(version = "current", raking = subnational_raking)
      )$shpfile_type
    } else {
      adm0_type <- detect_adm_shapefile_date_type(
        lsae.utils::get_admin_shapefile(version = shapefile_version, raking = subnational_raking)
      )$shpfile_type
    }
  }

...
#' @description Loads in a shapefile and simplifies it for use in creating the `simple_raster`.
#'
#' @param gaul_list a list of LBD standard location ids. Defines which countries to keep when building polygon
...
#' @param raking Boolean default F. If T, pulls the raking shapefile rather than the standard adm0 shapefile. Used in nonfractional raking.
...
load_simple_polygon <- function(gaul_list, buffer, tolerance = 0.2,
  ...
  if (is.null(custom_shapefile_path) & is.null(custom_shapefile)) {
    message("Opening reference shapefile...")
    reference_shape <- readOGR(lsae.utils::get_admin_shapefile(
      admin_level = 0, raking = raking,
      version = shapefile_version
    ))
    reference_shape@data$ADM0_CODE <- as.numeric(as.character(reference_shape@data$ADM0_CODE))
    subset_shape <- reference_shape[reference_shape@data$ADM0_CODE %in% gaul_list, ]
  } else if (!is.null(custom_shapefile_path) & is.null(custom_shapefile)) {
    message("Opening custom shapefile...")
    reference_shape <- readOGR(custom_shapefile_path)
    subset_shape <- reference_shape
  } else if (is.null(custom_shapefile_path) & !is.null(custom_shapefile)) {
    reference_shape <- custom_shapefile
    subset_shape <- reference_shape
  }
  ...
  if (subset_only == FALSE) {
    ...
    check_if_in_spoly <- function(the_poly, compare_to, the_proj = projection(reference_shape)) {
    ...
    projection(spoly_spdf) <- projection(reference_shape)

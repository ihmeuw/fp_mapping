...
#'
#' @note Takes a dataset with polygon records in specific format.
...
resample_polygons <- function(data,
  ...
  if (is.null(custom_ras) == F) {
    ...
    pars <- unique(d) %>% as.data.table()
    pars[, index := 1] # 2010 population raster
  ...
  # build simple raster and use as template
  simple_raster <- build_simple_raster(
    extent_template = simpp$subset_shape,
    shapefile_version = shapefile_version,
    reg = "all"
  )
  ...
  if (is.null(pop_release)) {
    ...
    pop_rast_path <- helper$covariate_paths(
      covariates = "worldpop",
      measures = "total",
  ...
  popraster <- load_worldpop_covariate(
    template_raster = simple_raster,
    covariate = "worldpop",
    pop_measure = "total",
  ...
  if (use_1k_popraster) {
    popraster <- disaggregate(popraster, 5) # needs to be 1km for density 0.001
  }
  ...
  if (is.null(custom_ras) == TRUE) { # use pop raster by default for resampling
    ...
    resample_ras <- raster::crop(custom_ras, extent(popraster))
  }

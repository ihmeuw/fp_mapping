...
predict_mbg <- function(res_fit,
                        ...
                        simple_raster_timebyadm1 = NULL,
                        simple_raster_timebyadm2 = NULL,
                        adm1_codes = NULL,
                        adm2_codes = NULL,
                        seed = NULL) {
  ...
  save(list = "draws", file = paste0("<<<< FILEPATH REDACTED >>>>", "/",name, ".RData"))
  ...
  ## get samples from adm1/adm2 time effects
  if (use_timebyadm1) {
    timebyadm1.res.idx <- grep("timeby_ADM1_CODE", par_names)
    pred_timebyadm1 <- sapply(draws, function(x) x$latent[timebyadm1.res.idx])
    rownames(pred_timebyadm1) <- CJ(period = 1:nperiod, adm1_codes)[, paste(period, adm1_codes)]

    timebyadm1.prec.idx <- grep("timeby_ADM1_CODE", names(draws[[1]]$hyperpar))
    pred_timebyadm1_prec <- sapply(draws, function(x) x$hyperpar[[timebyadm1.prec.idx]])

  } else {
    pred_timebyadm1 <- NULL
  }

  if (use_timebyadm2) {
    timebyadm2.res.idx <- grep("timeby_ADM2_CODE", par_names)
    pred_timebyadm2 <- sapply(draws, function(x) x$latent[timebyadm2.res.idx])
    rownames(pred_timebyadm2) <- CJ(period = 1:nperiod, adm2_codes)[, paste(period, adm2_codes)]

    timebyadm2.prec.idx <- grep("timeby_ADM2_CODE", names(draws[[1]]$hyperpar))
    pred_timebyadm2_prec <- sapply(draws, function(x) x$hyperpar[[timebyadm2.prec.idx]])

  } else {
    pred_timebyadm2 <- NULL
  }
  ...
  ## add on adm1/adm2 time effects, if present
  if (use_timebyadm1) {
    cell_timebyadm1 <- matrix(0, ncol = ncol(cell_l), nrow = nrow(cell_l))

    timebyadm1.vec <- as.character(values(simple_raster_timebyadm1)[cell_idx])
    timebyadm1.vec <- unlist(lapply(1:nperiod, function(x) ifelse(is.na(timebyadm1.vec), NA, paste(x, timebyadm1.vec))))
    cell_timebyadm1[!is.na(timebyadm1.vec), ] <- pred_timebyadm1[na.omit(timebyadm1.vec), ]

  }
  if (use_timebyadm2) {
    cell_timebyadm2 <- matrix(0, ncol = ncol(cell_l), nrow = nrow(cell_l))

    timebyadm2.vec <- as.character(values(simple_raster_timebyadm2)[cell_idx])
    timebyadm2.vec <- unlist(lapply(1:nperiod, function(x) ifelse(is.na(timebyadm2.vec), NA, paste(x, timebyadm2.vec))))
    cell_timebyadm2[!is.na(timebyadm2.vec), ] <- pred_timebyadm2[na.omit(timebyadm2.vec), ]
  ...
  ## get uncertainty from adm1/adm2 time effects
  if (use_timebyadm1) {
    cell_timebyadm1_sd <- apply(cell_timebyadm1, 1, sd)
  }
  if (use_timebyadm2) {
    cell_timebyadm2_sd <- apply(cell_timebyadm2, 1, sd)
  }
  ...
  # add adm1/adm2 time effect if present
  if (!is.null(pred_timebyadm1)) {
    cell_sd <- cell_sd + cell_timebyadm1_sd
  }
  if (!is.null(pred_timebyadm2)) {
    cell_sd <- cell_sd + cell_timebyadm2_sd
  }
  ...
  # add adm1/adm2 time effects if present
  if (use_timebyadm1) {
    cell_all <- cell_all + cell_timebyadm1
  }
  if (use_timebyadm2) {
    cell_all <- cell_all + cell_timebyadm2
  }

...
#' @param sharedir sharedir       <- file.path(fp_list$mbg_root, indicator_group, indicator)
...
#' @param gbd_pops output from "get_population" function
...
#' @param path_addin a suffix for the filename used when additional loopvars (aka age or holdouts)
#' are turned on. This is used when saving out the raking factors file in calculate_fractional_rfs
#' to distinguish the filenames for fractional_agg_rates
#' @param custom_population_function A function for generating custom population draws using the
#' inputs from `custom_population_list`. See the vignette `Using population draws` under the
#' `Working with the Package` tab.
#' @param custom_population_list A named list of paths to cell pred objects used to make custom
#' population draws. See docs named above.
#'
#' @return automatically saves out two raking factors tables, one for population and one for the
#' indicator. Returns `use_pop_draws`, a boolean which determines whether to rake/aggregate using
#' custom population draws rather than the GBD raked population vector (see docs listed under
...
fractional_rake_rates <- function(cell_pred = cell_pred,
  ...
  if (!(rake_method %in% c("linear", "logit"))) {
    stop("rake_method arg to fractional_rake_rates must be either 'linear' or 'logit")
  }

  # load the cell id to admin units link
  link_table <- get_link_table(simple_raster, shapefile_version = shapefile_version, raster_agg_factor = raster_agg_factor)
  ...
  if (!is.null(custom_population_function)) {
    # Adding on population draws to the cell pred
    cell_pred <- prep_custom_population_draws(
      ...
      sharedir = sharedir,
      custom_output_folder = custom_output_folder,
      stratum = paste0(reg, path_addin),
      indicator = indicator,
      run_date = run_date)
    use_pop_draws <- T
  } else {
    # need this for the function call for fractional_agg_rates
    use_pop_draws <- F
  }

...
#' are turned on. This is used when saving out the raking factors file to distinguish them for
#' fractional_agg_rates
#' @param use_pop_draws boolean, use draws of population? (see `custom_population_function` in
...
calculate_fractional_rfs <- function(ndraws = ndraws,
  ...
  if(use_pop_draws) {
    overs <- 1:ndraws
    draw_overs <- paste0("V", overs)
    pop_overs <- paste0("P", overs)

    # convert to counts, e.g. V1 * P1, v2 * P2
    cell_pred <- cell_pred[, (draw_overs) := lapply(overs, function(x) get(paste0("V", x)) * get(paste0("P", x)))]

    # aggregate draws and populations to country-year
    rake_geo <- cell_pred[, lapply(c(draw_overs, pop_overs),
                                   function(x) sum(get(x), na.rm = T)),
                          by = c("year", "location_id")]
    setnames(rake_geo, c("year", "location_id", draw_overs, pop_overs))

    # convert back to rates/prevalence
    rake_geo <- rake_geo[, (draw_overs) := lapply(overs, function(x) get(paste0("V", x)) / get(paste0("P", x)))]
  ...
  if (rake_method == "logit") {
    ...
    rf <- lapply(seq(nrow(cys)), function(x) {
      ...
      if (nrow(rake_geo[location_id == theloc & year == theyear]) == 0) {
        ...
        # when using pop draws, need to pass a matrix else, a vector
        # this is passed through the LogitFindK and EvalDiff functions to NewEst, where the value
        # calculation differs slightly depending on the whether draws are used or not
        if(use_pop_draws) {
            weights <- as.matrix(cell_pred[location_id == theloc & year == theyear & !is.na(V1), paste0("V", 1:ndraws), with = F])
        } else {
            weights <- as.numeric(cell_pred[location_id == theloc & year == theyear & !is.na(V1), pop_raked])
        }
  ...
  if (!is.null(custom_output_folder)) {
    write.csv(fractional_rf, file = "<<<< FILEPATH REDACTED >>>>")
  } else {
    write.csv(fractional_rf, file = "<<<< FILEPATH REDACTED >>>>")
  }

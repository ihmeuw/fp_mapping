...
#'
#' ad0_shape <- readRDS(file.path("<<<< FILEPATH REDACTED >>>>/background_map_africa.rds"))
...
subnational_ts_plots <- function(ad0_df,
  ...
  if (plot_data == T & multiple_runs == F) {
    ...
    ## format point for plotting crosswalked v. initial data
    ad0_data[round(outcome,5) != round(outcome_pre,5) & point==0, point := as.factor(4)][round(outcome,5) != round(outcome_pre,5) & point==1, point := as.factor(5)]
    ad1_data[round(outcome,5) != round(outcome_pre,5) & point==0, point := as.factor(4)][round(outcome,5) != round(outcome_pre,5) & point==1, point := as.factor(5)]
    ad2_data[round(outcome,5) != round(outcome_pre,5) & point==0, point := as.factor(4)][round(outcome,5) != round(outcome_pre,5) & point==1, point := as.factor(5)]
    ad0_data$point <- factor(ad0_data$point, levels = c(0,1,4,5))
    ad1_data$point <- factor(ad1_data$point, levels = c(0,1,4,5))
    ad2_data$point <- factor(ad2_data$point, levels = c(0,1,4,5))
  }
  ...
  if ("ad0" %in% plot_levels & "africa" %in% tolower(ad0_map_regions)) world_shape_simple <- readRDS(file.path(fp_list["mbg_root"], "time_trend_plots/world_shape_simple.rds"))
  ...
  theme_empty <- theme_classic() +
    theme(
      ...
      ## added margin
      plot.title = element_text(hjust = 0.5, size = 30, face = "bold", margin=margin(10,10,10,10)),
  ...
  ihme_logo <- png::readPNG(file.path(fp_list["mbg_root"], "time_trend_plots/ihme_logo.png"))
  ...
  if ("ad0" %in% plot_levels) {
    ...
    for (i in 1:length(ad0_code_list)) {
      ...
      if (n_ad0s > 16 & n_ad0s <= 25) wrap_width <- 18
      if (n_ad0s > 25) wrap_width <- 18
      ...
      if (plot_data == T & multiple_runs == F) gg_ad0_ts <- time_series_data_xwalk(ad0_df_reg, ad0_data_reg, admin = 0, val_range, title_plot_size, ind_title)
      ...
      centroids$ADM0_CODE <- as.numeric(centroids$ADM0_CODE)
      centroids <- merge(centroids, subset(plot_df, select = c("ADM0_CODE", "ihme_lc_id")), by="ADM0_CODE")
  ...
  if ("ad1" %in% plot_levels) {
    ...
    for (ctry in sort(countries)) {
      ...
      if (n_ad1s > 16 & n_ad1s <= 25) wrap_width <- 18
      if (n_ad1s > 25) wrap_width <- 18
      ...
      ad1_df_plot[is.na(ADM1_CODE), ADM1_CODE := 99]
      ...
      ## add a loop for 16 plots
      location_list <- unique(ad1_df_plot$ADM1_CODE)
      location_list <- location_list[location_list != 99]
      x <- seq_along(location_list)
      split_locations <- split(location_list, ceiling(x/15))

      for(ii in 1:length(split_locations)) {
        split_locations_new <- c(99, split_locations[[ii]])

        gg_ad1ts <- time_series_data_xwalk(ad1_df_plot[ADM1_CODE %in% split_locations_new], ad1_data_ctry[ADM1_CODE %in% split_locations_new], admin = 1, val_range, title_plot_size, ind_title)
        ...
        grid.draw(master_plot)
        if(ii != length(split_locations)) plot.new()
      }
  ...
  if ("ad2" %in% plot_levels) {
    ...
    for (ctry in sort(countries)) {
      ...
      for (ad1_code in ad1_table$ADM1_CODE) {
        ...
        location_list <- unique(plot_df$ADM2_CODE)
        location_list <- location_list[location_list != 99]
        x <- seq_along(location_list)
        split_locations <- split(location_list, ceiling(x/15))
        ...
        for(ii in 1:length(split_locations)) {
          ...
          gg_ad2ts <- time_series_data_xwalk(plot_df[ADM2_CODE %in% split_locations_new], ad2_data_ad1[ADM2_CODE %in% split_locations_new], admin = 2, val_range, title_plot_size, ind_title)
        ...
        if (multiple_runs == F) ad2_ad1 <- suppressWarnings(merge(ad2_ad1, plot_df2, duplicateGeoms = T))

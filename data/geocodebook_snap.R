## ########################################################################################################################################
##
## GEOCODEBOOK SNAP FUNCTIONS
## Purpose: Identifies points in codebook that are <15km away from country borders
...
format_codebook <- function(codebook){
  ...
  point.locations$iso3 <- gsub("_.*", "", point.locations$iso3)
  point.locations$latitude <- as.numeric(as.character(point.locations$lat))
  point.locations$longitude <- as.numeric(as.character(point.locations$long))
...
snap_calculate_points <- function(codebook, admin0){
  ...
  for (i in 1:length(countries)){
    ...
    sub.shapefile <- admin0[admin0@data$ISO3 == sub$iso3[1], ] 
...
# For a given codebook, identifies out-of-bound points <15km from border 
...
snap_codebook <- function(codebook) {
  ...
  out_of_bounds_over_15 <- data.frame()
  ...
  for (i in 1:10){
    if (dim(to_snap)[1] > 0){
      ...
      if (dim(out_of_bounds_snapped)[1] > 0) {
        if (i == 1){
          out_of_bounds_over_15 <- subset(out_of_bounds_snapped, min_distance_km > 15)
        }
        out_of_bounds_snapped <- subset(out_of_bounds_snapped, min_distance_km <= 15)
        if (dim(out_of_bounds_snapped)[1] > 0) {
  ...
  if (exists('out_of_bounds_over_15') && dim(out_of_bounds_over_15)[1]>0){
    point.locations$snapped[point.locations$ID %in% out_of_bounds_over_15$ID] <- "Over 15 km away"
  } 
  ...
  final_codebook$lat <- final_codebook$latitude
  final_codebook$long <- final_codebook$longitude
  final_codebook$latitude <- NULL
  final_codebook$latitude <- NULL
